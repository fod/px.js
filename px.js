var Px = (function () {
	...
	datatable: function(s) {
	    ...
	    var grpIdxs = [];
	    _.each(s, function(el, i) { if (el === '*') grpIdxs.push(i); });
	...
	explicitDatatable: function() {
	    // TODO -- check for data array size and if too big either (a) split the job, (b) throw an exception and suggest a truncate or subset operation first

	    var counts = this.valCounts(),
	        vars = this.variables(),
	        valIdx = arrayOfZeroes(counts.length),
	...
	truncate: function(s) {
		...
		// drop the values
	    	this.metadata.VALUES[this.variables()[i]] = 
	    	    _.filter(this.metadata.VALUES[this.variables()[i]], function(e, j) {
			return _.indexOf(s[i], j) !== -1;
		    });
	    }
		...
		if (c.length > 1) {
			...
			return _.include(want, d) ? p : arrayOfZeroes(multiple);
		    }));
